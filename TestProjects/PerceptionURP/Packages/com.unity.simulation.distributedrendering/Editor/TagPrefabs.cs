...
public class TagPrefabs : MonoBehaviour
{
    ...
    [MenuItem("Simulation/Distributed Rendering/Remove Tags")]
    public static void RemoveGameObjectIdComponent()
    {
        var prefabs = GetAllPrefabs();
        ...
        foreach (var prefabPath in prefabs)
        {
            ...
            {
                Debug.Log("Adding Component to Prefab: " + prefabPath);
                var gameObjectId = prefab.GetComponent<GameObjectId>();
                if (gameObjectId != null)
                    DestroyImmediate(gameObjectId);
                EditorUtility.SetDirty(prefab);
            }
