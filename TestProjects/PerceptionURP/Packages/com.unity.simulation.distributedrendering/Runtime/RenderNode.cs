...
namespace Unity.Simulation.DistributedRendering.Render
{
    public class RenderNode : MonoBehaviour
    {
                        ...
                        while (ms.Position < ms.Length)
                        {
                            ...
                            switch (msg)
                            {
                                ...
                                case MessageType.SetTransform:
                                {
                                    ...
                                    {
                                        Log.E($"ParseAndApply No GameObjectID for ID {id} Creating an empty gameobject");
                                        var go = new GameObject();
                                        var gameObjectId = go.AddComponent<GameObjectId>();
                                        gameObjectId.uniqueId = id;
                                        _objectsToSync.Add(id, gameObjectId);
                                    }
        ...
        private static void UpdatePlayerLoopAssetIfRequired(ref PlayerLoopAsset splAsset)
        {
            ...
            {
                var loopSystems = currentPlayerLoop.subSystemList[i].subSystemList.ToArray();
                if (currentPlayerLoop.subSystemList[i].type == typeof(Update) && loopSystems.Any())
                {
                    currentPlayerLoop.subSystemList[i].subSystemList = new PlayerLoopSystem[] {};
                }
            }
